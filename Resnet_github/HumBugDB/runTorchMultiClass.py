...
def build_dataloader(x_train, y_train, x_val=None, y_val=None, shuffle=True):
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=hyperparameters.batch_size, shuffle=shuffle,drop_last=True
    )
...
def train_model(
    ...
    for e in range(hyperparameters.epochs):
        ...
        train_acc = balanced_accuracy_score(
            np.argmax(all_y.numpy(),axis=1), np.argmax((all_y_pred.numpy() > 0.5).astype(float),axis=1)
        )
